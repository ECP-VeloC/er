  ...
  int tag)
{
  ...
  void* scratch = split_malloc(type_size);

  /* conduct the bitonic sort on our values */
  int ranks;
  MPI_Comm_size(comm, &ranks);
  int rc = sort_bitonic_sort(
  ...
  int recv_ints[7])
{
  ...
  void* left_buf = split_malloc(type_size);

  /* allocate a scratch buffer to receive value from right neighbor */
  void* right_buf = split_malloc(type_size);
  ...
  sort_bitonic(
    send_ints, result_type, result_type_size, 0, cmp_int,
    comm, tag1
  );
...
/* int rank_str(MPI_Comm comm, const void* str, int* groups, int* groupid)
...
int mpi_rank_str(MPI_Comm comm, const char* str, int* groups, int* groupid)
{
  ...
  void* buf = split_malloc(buf_size);
  ...
  *ptr = rank;
  ...
  sort_bitonic(
    buf, type, type_size, data_offset, cmp_str_int,
    comm, tag1
  );
  ...
  int recv_ints[7];
  split_sorted(
    buf, type, type_size, data_offset, cmp_str,
